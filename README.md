...
- **先退出旧版，再安装新版**。macOS 打开新的 DMG，把 Eternal 拖入「应用程序」并确认替换；Windows 直接运行新版安装包覆盖当前用户的旧安装。
- **不要先卸载后删除应用数据目录**。程序文件和任务数据分开保存；替换应用包不会删除已有任务、设置或密码库数据。
- 每次版本升级会先复制 `tasks.json` 到应用数据目录的 `backups/`，原文件保持在原位置。遇到无法创建快照、冲突快照或更高数据格式时，Eternal 会进入写保护，不会用空列表覆盖原数据。
- 当前版本没有自动更新器，需要手动安装新包；安装过程中请保持旧版本退出，避免两个版本同时写同一份数据。
