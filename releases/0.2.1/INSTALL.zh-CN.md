...
1. 运行 `Eternal-0.2.1-Windows-x64-Setup.exe`。
...
1. 打开 `Eternal-0.2.1-macOS-arm64.dmg`，把 Eternal 拖入“应用程序”。
...
- **提醒**：每行都有铃铛按钮；没有提醒时显示安静的铃铛，有提醒时显示时间或重复间隔。用鼠标点铃铛，或在键盘选中行后按 `Enter`，可打开提醒编辑器。
- **删除**：行内垃圾桶按钮，或在键盘选中行后按 `Delete` / `Backspace`，会先出现简洁确认层。`Enter` 确认删除，`Esc` 取消。删除任务会一并移除其提醒。
- `Esc` 会按层级取消快捷键录制、关闭弹层（含提醒与删除确认）、退出搜索，最后隐藏面板。
