...
- 面板、输入框和任务行采用低对比度层级与克制圆角；无边框原生窗口和页面画布
  保持透明，并关闭原生窗口阴影（`shadow: false`），让最外层 `14 px` 圆角真正
  裁掉四角，而不是露出矩形底色或 OS 矩形阴影。面板用 1px 边框做分离，不使用
  铺满窗口的外阴影。
...
- 固定底部快捷键帮助按上下文收敛，只显示当前有效动作：
  - 捕获态：`Enter 添加`、`↓ 列表`、`/ 搜索`、`⌘/Ctrl+1/2`、`Esc`
  - 键盘选中行：`Space 完成/恢复`、`Enter 提醒`、`⌫ 删除`、`↑ 输入`、`Esc`
  - 提醒：`Tab 切换`、`Enter 保存`、`Esc 取消`
  - 删除：`Enter 确认删除`、`Esc 取消`
  - 设置：`Esc 关闭`；录制快捷键：`Esc 取消录制`
  - 搜索：`↓ 结果`、`Space 完成/恢复`、`Esc 退出搜索`；结果行选中时 `↑ 搜索`
...
- 未提交草稿在应用进程存活期间保留。
- 设置中提供紧凑的「开机时启动 Eternal」开关（默认关闭，可键盘操作）。仅通过
  官方 Tauri v2 autostart 插件注册（`--autostart` 参数）；以 OS 注册状态为准；
  成功后才提交开关，失败保留原状态并显示简短行内错误；读取失败保持未知并提供
  重试。开机自启须保持静默：托盘/提醒/快捷键仍初始化，但跳过初始
  `show_panel`/抢焦点；主窗口 `visible: false`，无 Dock/任务栏图标。手动启动
  仍显示面板；托盘/快捷键唤出时通过 `panel-shown` 回到捕获。
...
- 云端服务和遥测。
- 自动更新与发布流程。
- 自定义 registry / LaunchAgent 实现（须使用官方 autostart 插件）。
